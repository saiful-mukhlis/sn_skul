 ...
 * @property Wall[] $walls
 */
class Mapel extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('usrid, code, name', 'required'),
			array('usrid, status', 'numerical', 'integerOnly'=>true),
			array('code, name', 'length', 'max'=>100),
			array('code', 'unique'),
			array('created, updated', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, usrid, code, name, created, updated, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'groupStudies' => array(self::HAS_MANY, 'GroupStudy', 'mapelid'),
			'usr' => array(self::BELONGS_TO, 'Usr', 'usrid'),
			'walls' => array(self::HAS_MANY, 'Wall', 'mapelid'),
		);
	}
